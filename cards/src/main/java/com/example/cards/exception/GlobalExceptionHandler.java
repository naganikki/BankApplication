...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<ErrorResponseDto> handleCustomerAlreadyExistsException(
			CardsAlreadyExistsException alreadyExistsException, WebRequest request) {
		ErrorResponseDto dto = new ErrorResponseDto(request.getDescription(true), HttpStatus.BAD_REQUEST,
	...
	@ApiResponses({ @ApiResponse(responseCode = "200", description = "account deleted"),
			@ApiResponse(responseCode = "500", description = "internal server error") })
	public ResponseEntity<ErrorResponseDto> handleException(CardsAlreadyExistsException alreadyExistsException,
			WebRequest request) {
		ErrorResponseDto dto = new ErrorResponseDto(request.getDescription(true), HttpStatus.INTERNAL_SERVER_ERROR,
				alreadyExistsException.getMessage(), LocalDateTime.now());
	...
	public ResponseEntity<ErrorResponseDto> handleCustomerNotFoundException(
		...
		ErrorResponseDto dto = new ErrorResponseDto(request.getDescription(true), HttpStatus.NOT_FOUND,
				alreadyExistsException.getMessage(), LocalDateTime.now());
